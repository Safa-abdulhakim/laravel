...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card-dark p-4">
            <h5 class="fw-bold text-white mb-4"><i class="bi bi-plus-circle me-2" style="color:#7c3aed"></i>{{ __('new_prompt_title') }}</h5>
            <form method="POST" action="{{ route('my-prompts.store') }}">
                @csrf
                <div class="row g-4">
                    <div class="col-12">
                        <label class="form-label">{{ __('title_label') }} <span style="color:#ef4444">*</span></label>
                        <input type="text" name="title" value="{{ old('title') }}" class="form-control form-control-dark @error('title') is-invalid @enderror" placeholder="{{ __('title_placeholder') }}">
                        @error('title')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12">
                        <label class="form-label">{{ __('content_label') }} <span style="color:#ef4444">*</span></label>
                        <textarea name="prompt_content" rows="8" class="form-control form-control-dark @error('prompt_content') is-invalid @enderror" placeholder="{{ __('content_placeholder') }}">{{ old('prompt_content') }}</textarea>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="status" class="form-select form-select-dark @error('status') is-invalid @enderror">
                            <option value="public" {{ old('status') == 'public' ? 'selected' : '' }}>{{ __('status_public') }}</option>
                    ...
                    <div class="col-12 d-flex gap-3 pt-2">
                        <button type="submit" class="btn btn-gradient px-4"><i class="bi bi-check2 me-2"></i>{{ __('create_prompt_btn') }}</button>
                        <a href="{{ route('my-prompts.index') }}" class="btn" style="background:rgba(148,163,184,0.1);color:#94a3b8;border:1px solid #334155;border-radius:10px;padding:10px 20px;">{{ __('cancel') }}</a>
                    </div>
                </div>
            </form>
        </div>
