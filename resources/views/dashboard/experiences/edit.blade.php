        ...
        <form method="POST" action="{{ route('admin.experiences.update', $experience) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="company_name" value="{{ old('company_name', $experience->company_name) }}" required class="form-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.company') }} (AR)</label>
                    <input type="text" name="company_name_ar" value="{{ old('company_name_ar', $experience->company_name_ar) }}" class="form-input" dir="rtl">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.position') }} (EN) *</label>
                    <input type="text" name="position" value="{{ old('position', $experience->position) }}" required class="form-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.position') }} (AR)</label>
                    <input type="text" name="position_ar" value="{{ old('position_ar', $experience->position_ar) }}" class="form-input" dir="rtl">
                </div>
            </div>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.start_date') }} *</label>
                    <input type="date" name="start_date" value="{{ old('start_date', $experience->start_date->format('Y-m-d')) }}" required class="form-input">
                </div>
                <div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="location" value="{{ old('location', $experience->location) }}" class="form-input">
            </div>
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.description') }} (EN)</label>
                    <textarea name="description" rows="4" class="form-input resize-none">{{ old('description', $experience->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">{{ __('dashboard.description') }} (AR)</label>
                    <textarea name="description_ar" rows="4" class="form-input resize-none" dir="rtl">{{ old('description_ar', $experience->description_ar) }}</textarea>
                </div>
            </div>
