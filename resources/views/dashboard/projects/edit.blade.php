        ...
        <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $project->title) }}" required class="form-input" placeholder="Project title">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Title (AR)</label>
                    <input type="text" name="title_ar" value="{{ old('title_ar', $project->title_ar) }}" class="form-input" dir="rtl" placeholder="اسم المشروع">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Description (EN)</label>
                    <textarea name="description" rows="4" class="form-input resize-none" placeholder="Short description...">{{ old('description', $project->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Description (AR)</label>
                    <textarea name="description_ar" rows="4" class="form-input resize-none" dir="rtl" placeholder="وصف مختصر...">{{ old('description_ar', $project->description_ar) }}</textarea>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Overview (EN)</label>
                    <textarea name="overview" rows="4" class="form-input resize-none" placeholder="Detailed overview...">{{ old('overview', $project->overview) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Overview (AR)</label>
                    <textarea name="overview_ar" rows="4" class="form-input resize-none" dir="rtl">{{ old('overview_ar', $project->overview_ar) }}</textarea>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Features (EN)</label>
                    <textarea name="features" rows="4" class="form-input resize-none" placeholder="- Feature 1&#10;- Feature 2">{{ old('features', $project->features) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Challenges (EN)</label>
                    <textarea name="challenges" rows="4" class="form-input resize-none" placeholder="Challenges faced...">{{ old('challenges', $project->challenges) }}</textarea>
                </div>
            </div>
            <div class="mb-6" x-data="{ techs: {{ json_encode(old('technologies', $project->technologies ?? [])) }}, newTech: '' }">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="url" name="github_link" value="{{ old('github_link', $project->github_link) }}" class="form-input" placeholder="https://github.com/...">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2" style="color: var(--color-heading);">Demo Link</label>
                    <input type="url" name="demo_link" value="{{ old('demo_link', $project->demo_link) }}" class="form-input" placeholder="https://...">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <div class="mb-2 flex items-center gap-3">
                        <img src="{{ Storage::url($project->cover_image) }}" class="w-20 h-20 rounded-xl object-cover">
                        <div>
                            <p class="text-xs mb-1" style="color: var(--color-muted);">Current cover image</p>
                            <label class="flex items-center gap-2 text-xs cursor-pointer" style="color: var(--color-danger);">
                                <input type="checkbox" name="remove_cover_image" value="1" class="w-3 h-3">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2 mb-2">
                        ...
                        <div class="relative group">
                            <img src="{{ Storage::url($img) }}" class="w-16 h-16 rounded-lg object-cover">
                            <label class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-lg cursor-pointer transition-opacity">
                    ...
                    <p class="text-xs mb-2" style="color: var(--color-muted);">Hover and click to remove existing images</p>
                    @endif
                    <input type="file" name="gallery[]" accept="image/*" multiple class="form-input">
                </div>
            </div>
            <div class="flex items-center gap-3 mb-6">
                <input type="checkbox" name="featured" id="featured" value="1" class="w-4 h-4"
