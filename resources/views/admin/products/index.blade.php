...
@section('page-subtitle', __('admin.manage_catalog'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <span class="text-muted small">{{ __('admin.total_count', ['count' => $products->total()]) }}</span>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        <th>#</th><th>{{ __('products.name') }}</th><th>{{ __('common.category') }}</th>
                        <th class="text-end">{{ __('common.price') }}</th><th class="text-center">{{ __('common.quantity') }}</th>
                        <th class="text-center">{{ __('common.status') }}</th><th class="text-end">{{ __('common.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <div class="btn-group btn-group-sm">
                                <a href="{{ route('admin.products.edit', $product) }}" class="btn btn-outline-primary" title="{{ __('common.edit') }}"><i class="bi bi-pencil"></i></a>
                                <form action="{{ route('admin.products.destroy', $product) }}" method="POST"
                                      onsubmit="return confirm('{{ __('common.confirm_delete') }}')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-outline-danger" title="{{ __('common.delete') }}"><i class="bi bi-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="text-center py-5 text-muted">
            ...
            <h5>{{ __('admin.no_products') }}</h5>
            <a href="{{ route('admin.products.create') }}" class="btn btn-primary mt-2">{{ __('admin.add_first') }}</a>
        </div>
        @endif
    </div>
</div>
