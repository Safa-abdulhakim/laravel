    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                ...
                <nav aria-label="breadcrumb" class="mb-4">
                    <ol class="breadcrumb" style="background:transparent">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('prompts.index') }}" class="text-muted text-decoration-none">Prompts</a></li>
                ...
                <div class="card-dark p-4 mb-4">
                    <div class="d-flex flex-wrap justify-content-between align-items-start mb-4">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            <span class="text-muted small"><i class="bi bi-eye me-1"></i>{{ $prompt->views }} views</span>
                            @auth
                                <form method="POST" action="{{ route('favorites.toggle', $prompt) }}" class="d-inline">
                                    ...
                                    <button type="submit" class="btn btn-sm" style="background:{{ $isFavorited ? 'rgba(239,68,68,0.15)' : 'rgba(71,85,105,0.3)' }};color:{{ $isFavorited ? '#ef4444' : '#94a3b8' }};border:1px solid {{ $isFavorited ? 'rgba(239,68,68,0.3)' : '#334155' }};border-radius:8px;">
                                        <i class="bi bi-heart{{ $isFavorited ? '-fill' : '' }} me-1"></i>{{ $isFavorited ? 'Saved' : 'Save' }}
                                    </button>
                                </form>
                            @endauth
                        </div>
                    ...
                    @if($prompt->tags->count())
                        <div class="mb-4">
                            <p class="text-muted small mb-2"><i class="bi bi-tags me-1"></i>Tags</p>
                    ...
                    <div class="d-flex flex-wrap gap-4 pt-3 border-top" style="border-color:#334155!important">
                        ...
                        <div class="text-muted small"><i class="bi bi-eye me-1"></i>{{ $prompt->views }} views</div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                    ...
                    @auth
                        <form method="POST" action="{{ route('favorites.toggle', $prompt) }}">
                            ...
                            <button type="submit" class="btn w-100" style="background:{{ $isFavorited ? 'rgba(239,68,68,0.15)' : 'rgba(71,85,105,0.2)' }};color:{{ $isFavorited ? '#ef4444' : '#e2e8f0' }};border:1px solid {{ $isFavorited ? 'rgba(239,68,68,0.3)' : '#334155' }};border-radius:10px;">
                                <i class="bi bi-heart{{ $isFavorited ? '-fill' : '' }} me-2"></i>{{ $isFavorited ? 'Remove from Favorites' : 'Add to Favorites' }}
                            </button>
                        </form>
                    @endauth
                </div>
                ...
                {{-- Prompt Details --}}
                <div class="card-dark p-4 mb-4">
                    <h6 class="fw-semibold text-white mb-3">Details</h6>
                ...
                @if($related->count())
                    <div class="card-dark p-4">
                        <h6 class="fw-semibold text-white mb-3">Related Prompts</h6>
                            ...
                            <a href="{{ route('prompts.show', $r) }}" class="text-decoration-none">
                                <div class="d-flex align-items-start gap-2 mb-3 pb-3" style="border-bottom:1px solid #1e293b">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-muted" style="font-size:0.75rem">{{ $r->views }} views</p>
                                    </div>
                                </div>
                            </a>
                        @endforeach
